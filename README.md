...
2. ให้ Agent รัน `powershell -ExecutionPolicy Bypass -File .\bootstrap-windows.ps1` เพื่อตรวจ Python, Git, Chrome, GitHub auth, dataset และ tests แล้วรัน `powershell -ExecutionPolicy Bypass -File .\scripts\bootstrap-analysis-windows.ps1` เพื่อเตรียม `.venv` และ X-13 แบบ repo-local สำหรับ analytical outputs ที่ต้องอัปเดตคู่กับ raw release
...
ขั้นนี้แยกจาก raw ingest โดยตั้งใจ และไม่แก้ `data/` หรือ `data.js`:
...
ผลลัพธ์อยู่ใน `derived/sa_pipeline_v3/` ครบทั้ง 30 cases สำหรับ `TH` และ `REG_ISAN5` พร้อม method log, rebase audit, X-13 diagnostics และ manifest ที่ผูกกับ hash ของ raw source ดูสัญญาวิธีคำนวณและกติกา fallback ฉบับเต็มที่ `analysis/README.md`

### ทางทดลอง/ทางสำรอง (Agent ห้ามเลือกเพื่อ publish เอง)
...
**สถานะที่ยืนยัน 2026-07-15:** Playwright profile แยกถูก Explore ใหม่หยุดที่ auth gate เพราะยังไม่ได้ลงชื่อเข้าใช้ Google; เปลี่ยน Chromium เป็น Chrome อย่างเดียวไม่ช่วย และหน้า classic ส่งเพียง `Year` 23 จุด จึงห้าม fallback หรือใช้ runner publish จนกว่าจะมี authenticated-profile smoke ที่ผ่าน

#### pytrends (งานเบา ไม่กี่คำ)
...
workflow `validate.yml` จะรัน tests, `audit.py --strict`, deterministic raw build check และ analytical output audit ทุก push/PR โดยไม่เขียนข้อมูล และตั้งใจไม่ใช้ `--require-latest` เพราะ code validation ต้องแยกจากรอบ refresh/publish
...
Agent ต้องเสนอเฉพาะ Monthly update ทางการก่อนเสมอ และขอผู้ใช้เฉพาะสิ่งที่ automation ทำแทนไม่ได้: ตั้ง Chrome ครั้งแรก, กด Import/Start และแก้ CAPTCHA หากพบ ไม่ควรโยนรายชื่อสคริปต์หรือทางทดลองทั้งหมดให้ผู้ใช้เลือก

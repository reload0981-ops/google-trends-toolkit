...
ใช้ Python มาตรฐาน ไม่ต้องติดตั้งอะไรเลย รู้จักทั้งไฟล์ export ของหน้าเว็บ GT (ไทย/อังกฤษ รายเดือน/รายสัปดาห์), ไฟล์ที่ตั้งชื่อ `<ID>__<GEO>.csv`, `manual_<ID>.csv` และ no-data manifest ไฟล์ CSV ต้องเริ่ม `2004-01` (TH) หรือ `2014-01` (จังหวัด), ต่อเนื่องถึงเดือนสมบูรณ์ล่าสุด และมีค่า finite 0–100 จึงจะเขียนทับคลังได้ ไฟล์ที่ไม่ผ่านจะถูกย้ายไป `incoming/review/` พร้อมเหตุผล ไม่มีการเดา

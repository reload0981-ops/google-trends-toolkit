...
async function processOne(job, settings) {
  ...
  if (response && response.result === "BLOCKED" && response.reason === "CAPTCHA") {
    try {
      await chrome.tabs.update(tab.id, { active: true });
...
async function handleCaptcha(job, resp) {
  log(
    `CAPTCHA detected on ${job.job_id}. Solve it in the opened tab, then click Resume.`,
    "err"
  );
  state.status = "paused";
  ...
  els.currentJob.textContent =
    `CAPTCHA - solve in opened tab, then click Resume\n` +
...
els.btnResume.addEventListener("click", async () => {
  ...
  if (state.captcha_tab_id) {
    ...
    log(`CAPTCHA tab closed (was ${state.captcha_tab_id}); retrying the job`, "ok");
